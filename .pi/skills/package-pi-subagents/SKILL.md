...
| Observation | `record-observer.ts`, `notification.ts`, `renderer.ts`, `subagent-events-observer.ts`, `composite-subagent-observer.ts`                                                                                                                                                        | 5       | Session-event stats, announce-only completion nudges (withheld during the parent's agent run and flushed on `agent_settled`; consumption is domain state on the record, and the sweep times session release from it), notification rendering, lifecycle-event emission, multi-observer fan-out |
