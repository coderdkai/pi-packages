...
| Observation | `record-observer.ts`, `notification.ts`, `renderer.ts`, `subagent-events-observer.ts`, `composite-subagent-observer.ts`                                                                                                                                                        | 5       | Session-event stats, announce-only completion nudges (consumption is domain state on the record; the sweep times session release from it), notification rendering, lifecycle-event emission, multi-observer fan-out                                                                       |
